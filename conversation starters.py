...
# Save the message in a text file for each group
groups = ["Group A", "Group B", "Group C"]  # example group names
for group in groups:
    with open(f"{group}_message.txt", "w") as file:
        file.write(f"Greetings {group} participants,\n\n")
        file.write(message)
        file.write("\nBest regards,\nThe organizers\n")
